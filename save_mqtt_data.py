...
def on_message(client, userdata, msg):
    try:
        ...
        if not all(len(lst) == num_readings for lst in [temperature, voltage, current, watts, status]):
        ...
        for i in range(num_readings):
            cursor.execute("""
                INSERT INTO sensor_data (device_id, device_timestamp, temperature, voltage, current, watts, status)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (device_id, device_timestamp[i], temperature[i], voltage[i], current[i], watts[i],  status[i]))
